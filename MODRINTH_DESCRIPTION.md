...
- **Blocked reasons** — An animal or villager that can't breed right now says why instead of
  falsely reading "✓ Ready": untamed, hurt, ridden, sitting, asleep, busy, or unable to breed at
  all (a mule, for example)
...
- **Dried ghasts** *(1.21.6+)* — Ripens through hydration stages while waterlogged: "Dried Ghast" → "Soaking" → "Ready to hatch!"
...
### Dolphins and Sulfur Cubes
- **Dolphin growth timers** — Baby dolphins get the same growth countdown as any other young animal; adults get no label, since dolphins do not breed. Not on `mc1.21-1.21.1`
- **Sulfur cube growth timers** *(mc26.2 only)* — Tracked the same way as any other baby-growing mob
- **Exact age from a bucket** — Bucketing a baby axolotl, tadpole or sulfur cube shows its exact remaining growth time as a tooltip on the bucket, carried into the label when released. Not on `mc1.21.2-1.21.5`
...
- **Compact mode** — A minimal HUD with lines shown only when relevant: Animals, Allays,
  Turtle Eggs, Sniffer Eggs, Dried Ghasts, Beehives, Crops, Villagers and Next Ready
...
All breedable animals in MC 26.2 are covered:

Cow, Mooshroom, Sheep, Pig, Chicken, Rabbit, Horse, Donkey, Mule, Llama, Wolf, Cat, Ocelot, Fox, Panda, Goat, Camel, Sniffer, Bee, Turtle, Axolotl, Frog, Strider, Hoglin, Armadillo, Nautilus, Dolphin, Sulfur Cube
...
- Show block labels — master switch for turtle eggs, sniffer eggs, dried ghasts, beehives and crops
...
Some features are version-gated: age lock (Golden Dandelion) is `mc26.1`/`mc26.2` only; nautilus is
`mc26.1`/`mc26.2` only (the entity class does not exist on Minecraft 1.21.9/1.21.10, which the
`mc1.21.11` branch's single jar also covers); camel husk is `mc26.1`, `mc26.2` and `mc1.21.11`;
...
- **Mod Loader**: Fabric (0.19.2+, requires Fabric API) or NeoForge
