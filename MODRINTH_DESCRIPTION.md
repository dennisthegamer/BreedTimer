...
### Dolphins
- **Dolphin growth timers** — Baby dolphins get the same growth countdown as any other young animal; adults get no label, since dolphins do not breed
...
All breedable animals in MC 1.21.2–1.21.5 are covered:

Cow, Mooshroom, Sheep, Pig, Chicken, Rabbit, Horse, Donkey, Mule, Llama, Wolf, Cat, Ocelot, Fox, Panda, Goat, Camel, Sniffer, Bee, Turtle, Axolotl, Frog, Strider, Hoglin, Armadillo, Dolphin
...
**This build**: Minecraft 1.21.2–1.21.5 (Fabric) / 1.21.3–1.21.5 (NeoForge), Java 21+.
...
exist on this build at all — the mobs and gates they add are absent here; camel husk is `mc26.1`,
`mc26.2` and `mc1.21.11` only, so it is not on this build either; dried ghast and happy ghast are
`mc1.21.6-1.21.8` and up only, so neither is on this build either; dolphin tracking is present (only
`mc1.21-1.21.1` lacks it). Exact bucket-release ages — the exact remaining
growth time other builds show on a bucketed baby axolotl or tadpole — are not on this build either:
two independent Minecraft API changes land inside this build's own version range at 1.21.5, so no
single implementation compiles across the whole range; a released baby here still starts from the
same estimate as any other. Lamb-eats-grass growth credit (feeding a baby sheep grass shortens its
timer) is **Fabric only** on this build: Minecraft moved the `Sheep` class into a new package at
1.21.5, inside this build's own version range, and NeoForge builds reference that package name
directly with no remap step, so it cannot safely target the class across the whole range the way
the Fabric build can. See the GitHub repository's `docs/PORTING.md` for the full table.
