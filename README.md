...
- Turtle egg and sniffer egg tracking, each with its own floating hatch-stage label
...
**This branch (`mc1.21.2-1.21.5`)**: Minecraft 1.21.2–1.21.5 (Fabric) / 1.21.3–1.21.5 (NeoForge).
...
or gates they add are present here; camel husk is `mc26.1`, `mc26.2` and `mc1.21.11` only, so it is
not on this branch either; dried ghast and happy ghast are `mc1.21.6-1.21.8` and up only, so neither
is on this branch either; dolphin tracking is present (it is only `mc1.21-1.21.1` that lacks it).
Exact bucket-release ages (the countdown a freshly released baby
axolotl or tadpole carries straight out of the bucket) exist on every other branch but not this one:
two independent version breaks land inside this branch's own Fabric range at 1.21.5 — `CompoundTag`'s
int reader changes return type, and `MobBucketItem`'s release hook changes its first parameter — so no
single implementation compiles across the whole range. Bucketed babies here fall back to the same
growth estimate every other baby uses. Lamb-eats-grass growth credit (feeding a baby sheep grass
shortens its timer) is **Fabric only** on this branch — Mojang moved the `Sheep` class into a new
package partway through this branch's own version range (1.21.5), and NeoForge's mods ship that
package name verbatim with no remap step, so a NeoForge build cannot target the class safely across
the whole range the way Fabric's remapped jar can. See `docs/PORTING.md` §2.2 for the detail.
...
Download the latest release from [Modrinth](https://modrinth.com/mod/breedtimer) or [GitHub Releases](https://github.com/DennisTheGamer/BreedTimer/releases). Each release contains one JAR per mod loader (`breedtimer-fabric-1.6.0+mc1.21.2-1.21.5.jar` and `breedtimer-neoforge-1.6.0+mc1.21.2-1.21.5.jar`).
...
- **Filter** - Five independent state checkboxes (Ready, Cooldown, Babies, In love, Blocked) that
  ...
  eggs, sniffer eggs, beehives and crops together
