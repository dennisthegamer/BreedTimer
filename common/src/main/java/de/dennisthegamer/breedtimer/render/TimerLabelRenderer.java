...
public class TimerLabelRenderer {
     ...
     *
     * <p>{@code null} in three cases: this panda cannot currently produce a cub (baby, age-locked,
     * never-grows or blocked -- {@link BreedCooldownHelper.AnimalState#READY}, {@code COOLDOWN} and
     ...
     */
    public static void renderAllayLabel(EntityRenderState state, Allay allay, PoseStack poseStack,
                                        SubmitNodeCollector collector, CameraRenderState camera) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null) return;
        ...
        renderText(state, allay.getBbHeight(), poseStack, collector, camera, mc,
     ...
     */
    public static void renderTadpoleLabel(EntityRenderState state, Tadpole tadpole, PoseStack poseStack,
                                          SubmitNodeCollector collector, CameraRenderState camera) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null) return;
        ...
        Component text = info.ageLocked()
                ? Component.translatable("breedtimer.state.age_locked")
                : info.remainingTicks() <= 0
        ...
        renderText(state, tadpole.getBbHeight(), poseStack, collector, camera, mc,
                text, null, fade, info.color(), config);
    }

    /**
     * Dolphins and, on 26.2, sulfur cubes. Like a tadpole they carry a growth estimate and nothing
     * else — neither species can breed (no {@code BreedGoal}, and {@code AgeableMob.canBreed()} is a
     * hard false), so an adult is given no label at all rather than a permanent "Cannot breed".
     */
    public static void renderAgeableLabel(EntityRenderState state, AgeableMob mob, PoseStack poseStack,
                                          SubmitNodeCollector collector, CameraRenderState camera) {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null) return;
        ...
        renderText(state, mob.getBbHeight(), poseStack, collector, camera, mc,
    ...
    private static void renderText(EntityRenderState state, float bbHeight, PoseStack poseStack,
        ...
        if (subLines != null) {
            ...
            for (int i = 0; i < subLines.size(); i++) {
                submitLine(poseStack, collector, mc, state, subLines.get(i),
    ...
    /**
     * Two passes, the way vanilla draws name tags -- its own submitNameTag still emits a NORMAL and
     * a SEE_THROUGH submit. The first carries the background box and ignores depth, so the label
     * never disappears behind the animal or the terrain; the second redraws the glyphs opaque on top
     * of that box. With only the first, the text sits under the background quad and is multiplied by
     * it: at the default 50% opacity that is exactly half brightness (#55FF55 measured as #2A7E2A)
     * and contrast falls to 1.4:1.
     */
    private static void submitLine(PoseStack poseStack, SubmitNodeCollector collector, Minecraft mc,
                                   EntityRenderState state, Component line, float y,
                                   float fade, int color, int backgroundColor, int outlineColor) {
        ...
        collector.submitText(poseStack, x, y, line.getVisualOrderText(), false,
                Font.DisplayMode.SEE_THROUGH, state.lightCoords, seeThroughColor, backgroundColor, 0);
        ...
        collector.submitText(poseStack, x, y, line.getVisualOrderText(), false,
                Font.DisplayMode.NORMAL, state.lightCoords, textColor, 0, outlineColor);
    }
    ...
    private static Component getDisplayComponent(AnimalTimerInfo info) {
        return switch (info.state()) {
            ...
            case BABY -> info.remainingTicks() <= 0
                    ...
                    : Component.translatable("breedtimer.state.growing",
                            BreedCooldownHelper.formatTime(info.remainingTicks()));
            case AGE_LOCKED -> Component.translatable("breedtimer.state.age_locked");
