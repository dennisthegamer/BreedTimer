...
public class BreedTimerHud {
    ...
    /**
     * Box-Maße einer Zeilenliste; von {@link #extractRenderState}, {@link #measureBox} und
     * {@link #draw} geteilt, damit Vermessung und Zeichnung nie auseinanderdriften (dieselbe
     * Invariante wie bei FishingStats: computeLayout() und draw() müssen dieselben Strings sehen).
     */
    ...
    private static MutableComponent driedGhastText(int[] counts) {
        StatePalette p = StatePalette.current();
        return Component.empty()
                .append(Component.translatable("breedtimer.hud.dried_ghasts").withColor(p.neutral))
                .append(Component.translatable("breedtimer.hud.dry", counts[0]).withColor(p.inert))
                .append(Component.translatable("breedtimer.hud.soaking", counts[1]).withColor(p.young))
                .append(Component.translatable("breedtimer.hud.ready_plain", counts[2]).withColor(p.ready));
    }
    ...
    private static List<MutableComponent> buildLines(BreedTimerConfig config, int ready, int cooldown, int babies,
                                                       int inLove, int blocked, int[] eggCounts, int[] snifferCounts, int[] ghastCounts,
        ...
        if (config.showAnimals) {
            ...
            int totalGhasts = ghastCounts[0] + ghastCounts[1] + ghastCounts[2];
            if (totalGhasts > 0) {
                lines.add(driedGhastText(ghastCounts));
            }
    ...
    // ── Vermessen + Zeichnen (von extractRenderState() UND drawPreview()/measureBox() geteilt) ──
     ...
     */
    private static void draw(GuiGraphicsExtractor graphics, int x, int y, Layout layout, Font font, int bgColor) {
        ...
        for (MutableComponent line : layout.lines()) {
            ...
            graphics.text(font, line, textX, currentY, 0xFFFFFFFF, true);
            currentY += font.lineHeight + 4;
        }
    }

    /** Draws {@code layout} at the unscaled coordinates (x,y) under a {@code scale} transform. */
    private static void drawScaled(GuiGraphicsExtractor graphics, int x, int y, Layout layout,
    ...
    public static void extractRenderState(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
        ...
        if (config.showAnimals) {
            ...
            for (Animal animal : animals) {
                ...
                switch (info.state()) {
                    ...
                    // Age-locked animals are still babies, just permanently so.
                    case BABY, AGE_LOCKED, NEVER_GROWS -> babies++;
        ...
        if (config.showVillagers) {
            for (Villager villager : level.getEntitiesOfClass(Villager.class, scanBox)) {
        ...
        // Dolphins and, on 26.2, sulfur cubes: AgeableMobs but not Animals, so the animal loop
        // never sees them either. Like tadpoles the babies count is the only thing they can
        // contribute -- neither species breeds. One AgeableMob query rather than one per species:
        // EntityTypeTest.forClass has no index, so a Dolphin.class query walks exactly the same
        // sections for the same per-entity instanceof, and two of them would be two full passes.
        if (config.showAnimals && config.showBabyTimer) {
            for (AgeableMob mob : level.getEntitiesOfClass(AgeableMob.class, scanBox,
                    AgeableTracking::isTrackedBaby)) {
                if (visible.test(mob)) babies++;
            }
        }
        ...
        int[] ghastCounts = showBlocks ? BlockLabelScanner.getDriedGhastCounts() : new int[] {0, 0, 0};
        ...
        List<MutableComponent> lines = buildLines(config, ready, cooldown, babies, inLove, blocked, eggCounts, snifferCounts, ghastCounts,
