...
public class BreedTimerHud {
    ...
    public static void render(GuiGraphics graphics, DeltaTracker tickCounter) {
        ...
        // Dolphins: AgeableMobs but not Animals, so the animal loop never sees them either. Like
        // tadpoles the babies count is the only thing they can contribute -- the species does not
        // breed. One AgeableMob query rather than one per species: EntityTypeTest.forClass has no
        // index, so a Dolphin.class query walks exactly the same sections for the same per-entity
        // instanceof, and two of them would be two full passes.
