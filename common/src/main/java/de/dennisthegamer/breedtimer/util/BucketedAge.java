 ...
 *   <li>{@code AgeLocked} does not exist on this version at all -- the golden dandelion that would
 *       set it is 26.x-only, so {@code ageLocked} is a constant false here.</li>
 * </ul>
 *
 * <p>{@code CompoundTag.getInt(String)} returns a plain {@code int} on this version rather than the
 * {@code Optional<Integer>} a later Minecraft version's default-valued reader hands back, and a missing
 * key returns 0 the same as a genuine {@code Age} of 0 -- so a preceding {@code contains} check is the
 * only way to tell "no age was ever written" from "the mob's age happens to be zero".
 */
public final class BucketedAge {
    ...
    public static Info read(ItemStack stack) {
        ...
        if (!tag.contains("Age")) return Info.UNKNOWN;
