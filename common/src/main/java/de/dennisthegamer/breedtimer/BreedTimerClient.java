...
public final class BreedTimerClient {
     ...
     *
     * <p>PORT WARNING (Task 45): {@code mc.gui.hud.setOverlayMessage} is the mc26.2-only spelling.
     * 26.2 split {@code Gui} into {@code Gui}/{@code Hud}; {@code Gui.hud} is the accessor and
     * {@code Gui} itself has no {@code setOverlayMessage}. Every other branch (mc26.1 and all four
     * 1.21.x branches) calls {@code mc.gui.setOverlayMessage(...)} directly, with no {@code .hud}.
     */
    private static void announce(Minecraft mc, String key) {
        // PORT WARNING (Task 45): mc26.2-only spelling -- mc.gui.hud.setOverlayMessage. The other
        // five branches (mc26.1 and all four 1.21.x) call mc.gui.setOverlayMessage(...), no .hud.
        mc.gui.hud.setOverlayMessage(Component.translatable(key), false);
    }
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        if (config.showAnimals) {
            // Readiness is remembered for every loaded animal, not just the visible ones. Keying it
            // to visibility meant looking away dropped them from the set, so looking back chimed
            // for each one all over again.
            //
            ...
            // ...but the chime itself still only fires for one you could actually see become ready.
            //
            // The scan below is skipped outright unless sound is on and some animal is both ready
            // and was not ready last tick, because nothing else can make it play a sound. That is
            // not an approximation: getVisibleAnimals returns a subset of the loaded animals just
            // walked, createTimerInfo reads only maps that nothing mutates between the two loops, so
            // an animal it reports as READY is necessarily in currentlyReady. With the difference to
            // previouslyReady empty, the loop provably cannot fire — and skipping it saves a scan
            // box query plus one line-of-sight raycast per visible animal, every tick.
            //
