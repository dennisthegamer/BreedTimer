...
- **"❤ Needs full health" now also covers wolves, nautiluses and owned cats.** A hurt wolf or
  ...
  correctly, do nothing about it. The nautilus itself only exists on Minecraft 1.21.11, and this
  build's single jar also covers 1.21.9/1.21.10 — so it is matched by registry id rather than by
  entity class, which costs nothing on the two versions where no nautilus can ever appear.
