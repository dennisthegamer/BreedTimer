...
- **Dolphin growth timers.** A baby dolphin now gets the same growth countdown as any other young
  animal; an adult dolphin gets no label at all, since dolphins do not breed. Not on `mc1.21-1.21.1`.
- **Sulfur cube growth timers.** `mc26.2` only — sulfur cubes do not exist on any other supported
  version.
...
- **Exact age from a bucketed animal.** Bucketing a baby axolotl, tadpole or (`mc26.2` only) sulfur
  cube shows its exact remaining growth time as a tooltip on the bucket — the one moment this
  client-only mod can ever know an exact age. Releasing it carries that same number into its
  floating label instead of starting over at a fresh estimate. Not on `mc1.21.2-1.21.5`.
