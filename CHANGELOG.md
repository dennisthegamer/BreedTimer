...
- **Exact age from a bucketed animal.** Bucketing a baby axolotl or tadpole shows its exact
  remaining growth time as a tooltip on the bucket — the one moment this client-only mod can ever
  know an exact age. Releasing it carries that same number into its floating label instead of
  starting over at a fresh estimate.
...
### Performance
- **Labels now skip more per-entity work when nothing could be drawn.** The label mixin used to run
  every rendered entity through the animal/villager/allay/tadpole checks unconditionally; it now
  rejects up front — before looking at entity type at all — in compact mode, with both Show Animals
  and Show Villagers off, or once an entity is past the fade-out distance. This branch's render hook
  already receives the entity directly rather than searching the world for it, so it never had the
  herd-scaling label mix-up other builds of this mod fix here — this is a smaller win: fewer checks
  paid by every rendered entity, every frame.
...
- The age lock (Golden Dandelion), sulfur cube and nautilus do not exist on this build at all —
  none of the labels, food hints or bucket handling those three add apply here. Camel husk, dried
  ghast and happy ghast tracking are likewise absent: none of those exist before `mc1.21.6-1.21.8`.
- A baby's growth is not tracked when that baby is a dolphin. At 1.21/1.21.1 a dolphin is a
  `WaterAnimal`, not the shared ageable superclass every other tracked baby's label hooks into, so
  it never becomes a candidate for a label or a bucket readout at all.
...
- Standardized jar naming to `<modid>-<loader>-<version>+mc<range>` (e.g. `breedtimer-fabric-1.4.0+mc1.21-1.21.1.jar`).
