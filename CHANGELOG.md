...
- **Breeding conditions are shown on the label.** Where an animal is blocked, the label names the
  ...
  never breeds at all — mule, happy ghast, polar bear, parrot, skeleton horse and zombie horse. The
  compact HUD gained a matching "n blocked" counter.
...
- Standardized jar naming to `<modid>-<loader>-<version>+mc<range>` (e.g. `breedtimer-fabric-1.4.0+mc1.21.6-1.21.8.jar`).
